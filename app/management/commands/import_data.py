...
from app.models import (Area, Coordenadoria, Diretoria, Employee,
                        EmployeeLevel, EmployeeType, Empresa, Gerencia, Person,
                        SurveyResponse)
            ...
            for row in reader:
                try:
                    ...
                    SurveyResponse.objects.get_or_create(
                        ...
                        defaults={
                            'interesse_no_cargo': int(row['Interesse no Cargo']),
                            'comentarios_interesse_no_cargo': row['Comentários - Interesse no Cargo'] or '',
                            'contribuicao': int(row['Contribuição']),
                            'comentarios_contribuicao': row['Comentários - Contribuição'] or '',
                            'aprendizado_e_desenvolvimento': int(row['Aprendizado e Desenvolvimento']),
                            'comentarios_aprendizado_e_desenvolvimento': row['Comentários - Aprendizado e Desenvolvimento'] or '',
                            'feedback': int(row['Feedback']),
                            'comentarios_feedback': row['Comentários - Feedback'] or '',
                            'interacao_com_gestor': int(row['Interação com Gestor']),
                            'comentarios_interacao_com_gestor': row['Comentários - Interação com Gestor'] or '',
                            'clareza_sobre_possibilidades_de_carreira': int(row['Clareza sobre Possibilidades de Carreira']),
                            'comentarios_clareza_sobre_possibilidades_de_carreira': row['Comentários - Clareza sobre Possibilidades de Carreira'] or '',
                            'expectativa_de_permanencia': int(row['Expectativa de Permanência']),
                            'comentarios_expectativa_de_permanencia': row['Comentários - Expectativa de Permanência'] or '',
                            'enps': int(row['eNPS']),
