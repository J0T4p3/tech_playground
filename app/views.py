...
class BaseReadOnlyModelViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter(name='page', type=int, description='Número da página (deve ser > 0)', required=False),
            OpenApiParameter(name='ordering', type=str, description='Campo de ordenação (ex.: nome, -nome)', required=False),
        ],
        responses={
            200: None,  # Will be auto-generated
            400: OpenApiResponse(description="Requisição Inválida - Paginação inválida (número da página <=0) ou parâmetros de ordenação"),
            404: OpenApiResponse(description="Não Encontrado - Página não encontrada"),
            500: OpenApiResponse(description="Erro Interno do Servidor - Erro inesperado do servidor"),
        }
    )
    ...
    @extend_schema(
        responses={
            200: None,  # Will be auto-generated
            400: OpenApiResponse(description="Requisição Inválida - Parâmetros de entrada inválidos"),
            404: OpenApiResponse(description="Não Encontrado - Objeto não encontrado"),
            500: OpenApiResponse(description="Erro Interno do Servidor - Erro inesperado do servidor"),
        }
    )
