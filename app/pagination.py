    ...
    def paginate_queryset(self, queryset, request, view=None):
        ...
        try:
            ...
            raise ValidationError(
                {
                    'detail': 'Número de página inválido. Deve ser um número inteiro.'
                }
            )

        if requested_page_number <= 0:
            raise ValidationError(
                {'detail': 'Número de página deve ser maior que 0.'}
            )
        ...
        if offset >= total_count and total_count > 0:
            raise ValidationError(
                {
                    'detail': f'Número de página {requested_page_number} excede o total de itens disponíveis.'
                }
            )
        ...
        if page is not None:
            ...
            if requested_page_number > total_pages:
                raise ValidationError(
                    {
                        'detail': f'Número de página {requested_page_number} excede o total de páginas disponíveis ({total_pages}).'
                    }
                )
