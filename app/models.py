    ...
    if isinstance(value, str):
        ...
        if stripped != value:
            raise ValidationError(
                'O valor não deve conter espaços em branco no início ou fim.'
            )
    return value
...
class Gerencia(models.Model):
    ...
    diretoria = models.ForeignKey(
        Diretoria, on_delete=models.CASCADE, verbose_name='Diretoria'
    )
...
class Coordenadoria(models.Model):
    ...
    gerencia = models.ForeignKey(
        Gerencia, on_delete=models.CASCADE, verbose_name='Gerência'
    )
...
class Area(models.Model):
    ...
    cordenadoria = models.ForeignKey(
        Coordenadoria, on_delete=models.CASCADE, verbose_name='Coordenadoria'
    )
...
class Person(models.Model):
    ...
    geracao = models.CharField(
        max_length=20,
        choices=GenerationChoices.choices,
    ...
    def clean(self):
        if (
            Person.objects.filter(email=self.email)
            .exclude(pk=self.pk)
            .exists()
        ):
            raise ValidationError(
                'Este email já está em uso por outra pessoa.'
            )
...
class Employee(models.Model):
    pessoa = models.OneToOneField(
        Person, on_delete=models.CASCADE, verbose_name='Funcionário'
    )
    ...
    email_corporativo = models.EmailField(
        verbose_name='Email Corporativo', unique=True, db_index=True
    )
    ...
    area = models.ForeignKey(
        Area, on_delete=models.CASCADE, verbose_name='Área', db_index=True
    )
    estado = models.CharField(
        max_length=2, choices=STATE_CHOICES, verbose_name='Estado'
    )
    tempo_de_empresa = models.CharField(
        max_length=20,
        verbose_name='Tempo de Empresa',
        validators=[validate_stripped],
    )

    def clean(self):
        if (
            Employee.objects.filter(email_corporativo=self.email_corporativo)
            .exclude(pk=self.pk)
            .exists()
        ):
            raise ValidationError(
...
class SurveyResponse(models.Model):
    ...
    )   # Relação com o funcionário que respondeu a pesquisa
    data_da_resposta = models.DateField(
        verbose_name='Respondido em', db_index=True
    )

    interesse_no_cargo = models.IntegerField(verbose_name='Interesse no cargo')
    comentarios_interesse_no_cargo = models.TextField(
        blank=True, verbose_name='Interesse no cargo - Comentários'
    )

    contribuicao = models.IntegerField(verbose_name='Contribuição')
    comentarios_contribuicao = models.TextField(
        blank=True, verbose_name='Comentários - Contribuição'
    )

    aprendizado_e_desenvolvimento = models.IntegerField(
        verbose_name='Aprendizado e Desenvolvimento'
    )
    comentarios_aprendizado_e_desenvolvimento = models.TextField(
        blank=True, verbose_name='Comentários - Aprendizado e Desenvolvimento'
    )
    ...
    interacao_com_gestor = models.IntegerField(
        verbose_name='Interação com o Gestor'
    )
    comentarios_interacao_com_gestor = models.TextField(
        blank=True, verbose_name='Comentários - Interação com o Gestor'
    )

    clareza_sobre_possibilidades_de_carreira = models.IntegerField(
        verbose_name='Clareza sobre Possibilidades de Carreira'
    )
    comentarios_clareza_sobre_possibilidades_de_carreira = models.TextField(
        blank=True,
        verbose_name='Comentários - Clareza sobre Possibilidades de Carreira',
    )

    expectativa_de_permanencia = models.IntegerField(
        verbose_name='Expectativa de Permanência'
    )
    comentarios_expectativa_de_permanencia = models.TextField(
        blank=True, verbose_name='Comentários - Expectativa de Permanência'
    )

    enps = models.IntegerField(verbose_name='eNPS')
    aberta_enps = models.TextField(
